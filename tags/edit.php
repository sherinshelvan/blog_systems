...
class Edit extends Common_Functions{
	...
	function __construct(){
		parent::__construct();
		$this->page_heading = 'Add Tag';
		$this->page_url     = BASE_URL.'tags/edit.php'.(isset($_GET['edit']) ? ("?edit=".$_GET['edit']) : "");
		$this->table_name   = TABLE_PREFIX.'tags';
		$this->return_url   = BASE_URL.'tags';
		$this->index();
	}
	...
	public function form_validation($id = ''){
		...
		if( (isset($form_data['name']) && $form_data['name'] == '') 
			...
			$this->error = true;
			$this->message = "<p>Tag Name Required</p>";
		}
		if(isset($form_data['name']) && $form_data['name'] != ''){
			...
			if($exist){
				$this->error = true;
				$this->message = "<p>Tag Name already exist</p>";
			}
		}
	}
				...
				<form action="<?=$obj->page_url?>" method="POST" enctype="multipart/form-data">				
					...
					<div class="row">
				        <div class="input-field col s12">
				          	<input placeholder="Tag" name="name"
